...
#   plt.show()
   ...
   if ptch=="min":
      ptch=inputFilename[-9:-4]
   if ptch=="#-min":
      ptch=inputFilename[-10:-4]
...
pt='sounds'
lst=fnmatch.filter(os.listdir(pt), '*_*.wav')


#lst=fnmatch.filter(os.listdir(pt), 'zykina_017_f-1.wav')
lst.sort()
for f in lst:
   print f
   anal_pitch(pt,f)
